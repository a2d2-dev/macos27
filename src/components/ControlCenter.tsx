...
function ToggleTile({ tile }: { tile: ToggleTileConfig }) {
  ...
  return (
    <button
      type="button"
      className={`flex h-[58px] min-w-0 items-center gap-3 rounded-2xl px-3 text-left outline-none transition ${
        enabled ? 'bg-white/55 shadow-sm' : 'bg-white/28 hover:bg-white/38'
...
function ActionTile({
  ...
  return (
    <button
      type="button"
      className={`flex h-11 items-center gap-3 rounded-2xl px-3 text-left text-[13px] outline-none transition ${
        active ? 'bg-white/60 shadow-sm' : 'bg-white/28 hover:bg-white/38'
      } focus-visible:ring-2 focus-visible:ring-white/70`}
      aria-pressed={active}
      onClick={onClick}
    >
...
export function ControlCenter() {
  ...
  return (
    <div
      ref={panelRef}
      className="glass-surface-strong fixed right-3 top-9 z-[10001] w-[318px] rounded-[22px] p-3 text-[var(--text-primary)]"
      role="dialog"
    ...
    >
      ...
      <section className="mt-2 rounded-2xl bg-white/28 p-3" aria-label="Display controls">
      ...
      <section className="mt-2 rounded-2xl bg-white/28 p-3" aria-label="Sound controls">
      ...
      <section className="mt-2 flex items-center gap-3 rounded-2xl bg-white/28 p-3" aria-label="Battery status">
        <span className="grid h-9 w-9 place-items-center rounded-full bg-white/45 text-emerald-500">
          <BatteryFull size={19} />
        </span>
        <div>
